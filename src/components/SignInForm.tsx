...
export default function SignInForm({ callbackUrl, providers }: SignInFormProps) {
  ...
  const handleSocialSignIn = async (provider: "github" | "google" | "unipi") => {
    setIsLoading(true);
    ...
    <div className="space-y-3">
          ...
          <form onSubmit={handleMagicLinkSignIn} className="space-y-4">
            ...
            {emailSent && (
              <p className="text-sm text-green-700">
                Abbiamo inviato un link di accesso a {email}.
              </p>
            )}
          </form>
        )}
        ...
        {providers.unipi && (
          <button
            type="button"
            disabled={isLoading}
            onClick={() => handleSocialSignIn("unipi")}
          ...
          >
            Accedi con le credenziali Unipi
          </button>
        )}
        {!providers.email && !providers.github && !providers.google && !providers.unipi && (
          <p className="text-center text-sm text-gray-600">
            Nessun provider di accesso e&apos; configurato.
          </p>
        )}
    </div>
  );
